...
import { eq, count, desc, isNull, isNotNull } from "drizzle-orm";
...
admin.get("/data", requireAdmin, async (c) => {
  ...
  const [linkCount, publicLinkCount, pendingCount, reportRows, publicLinkRows] = await Promise.all([
    db.select({ total: count() }).from(schema.shortLinks).where(isNotNull(schema.shortLinks.userId)),
    db.select({ total: count() }).from(schema.shortLinks).where(isNull(schema.shortLinks.userId)),
  ...
  return c.json({
    stats: {
      ...
      totalPublicLinks: publicLinkCount[0]?.total ?? 0,
